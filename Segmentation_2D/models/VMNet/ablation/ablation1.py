...
from ..TreeSSM.tree_ssm_block import GrootVBlock
...
class VMNet_ablation1(nn.Module):
    ...
    def __init__(
        ...
        :param dim:
        """
        super(VMNet_ablation1, self).__init__()
        self.device = device
        ...
        self.conv0x = EncoderConv(
            n_channels,
            self.number,
        )
        self.conv0y = EncoderConv(
            n_channels,
            self.number,
        )
        ...
        self.conv2x = EncoderConv(
            self.number,
            2 * self.number,
        )
        self.conv2y = EncoderConv(
            self.number,
            2 * self.number,
        )
        ...
        self.conv4x = EncoderConv(
            2 * self.number,
            4 * self.number,
        )
        self.conv4y = EncoderConv(
            2 * self.number,
            4 * self.number,
        )
        ...
        self.conv6x = EncoderConv(
            4 * self.number,
            8 * self.number,
        )
        self.conv6y = EncoderConv(
            4 * self.number,
            8 * self.number,
        )
        ...
        # #插入TreeSSM
        # self.tree_ssm = GrootVBlock(
        #     channels=8*self.number,
        #     depth=15,
        #     downsample=False,
        # )

        self.conv120 = EncoderConv(12 * self.number, 4 * self.number)
        self.conv12x = EncoderConv(
            12 * self.number,
            4 * self.number,
        )
        self.conv12y = EncoderConv(
            12 * self.number,
            4 * self.number,
        ...
        self.conv14x = EncoderConv(
            6 * self.number,
            2 * self.number,
        )
        self.conv14y = EncoderConv(
            6 * self.number,
            2 * self.number,
        )
        ...
        self.conv16x = EncoderConv(
            3 * self.number,
            self.number,
        )
        self.conv16y = EncoderConv(
            3 * self.number,
            self.number,
        )
    ...
    def forward(self, x):
        ...
        # # 插入TreeSSM
        # x_3_1 = x_3_1.permute(0, 2, 3, 1)
        # x_3_1 = self.tree_ssm(x_3_1)
        # x_3_1 = x_3_1.permute(0, 3, 1, 2)
