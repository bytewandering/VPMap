...
class DSC(object):
    ...
    """
    input: input feature map [N,C,D,W,H]；coordinate map [N,K*W,H]或[N,K*W,H] 
    ...
    def deform_conv(self, input, offset, if_offset):
        ...
        #[B,2*K+1,W,H,2]
        sampled_coor = torch.stack([y,x],dim=-1).double()
    
    
        """插值采样"""
        deformed_feature = []
        for i in range(0,self.num_points):
            deformed_feature.append(F.grid_sample(input,sampled_coor[:,i,:,:,:]))
        if self.morph == 0:
            #[B,C,W,2*K+1,H]
            deformed_feature = torch.stack(deformed_feature,dim=3)
            deformed_feature = deformed_feature.flatten(start_dim=2,end_dim=3)
        if self.morph == 1:
            #[B,C,W,H,2*K+1]
            deformed_feature = torch.stack(deformed_feature,dim=4)
            deformed_feature = deformed_feature.flatten(start_dim=3,end_dim=4)
