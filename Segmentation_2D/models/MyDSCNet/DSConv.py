...
class DSConv(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, kernel_size, extend_scope, morph,
        ...
        self.offset_conv = nn.Conv2d(in_ch, kernel_size, 3, padding=1)
    ...
    def forward(self, f):
        ...
        deformed_feature = efficient_deform_feature(f, offset,self.kernel_size//2,self.morph)
        if self.morph == 0:
...
def efficient_deform_feature(feature:torch.Tensor, offset, K, morph):
    ...
    if morph == 0:
        ...
        for i in range(0,K):
            offset_x[:,K+i+1,:,:] = offset_x[:,K+i,:,:]+offset[:,K+i+1,:,:]
            offset_x[:,K-i-1,:,:] = offset_x[:,K+i,:,:]+offset[:,K-i-1,:,:]
    elif morph == 1:
        ...
        for i in range(0,K):
            offset_y[:,K+i+1,:,:] = offset_y[:,K+i,:,:]+offset[:,K+i+1,:,:]
            offset_y[:,K-i-1,:,:] = offset_y[:,K-i,:,:]+offset[:,K-i-1,:,:]
    ...
    #[B,2*K+1,W,H,2]
    sampled_coor = torch.stack([sampled_coor_x,sampled_coor_y],dim=-1).double()
    ...
    if morph == 1:
        #[B,C,W,H,2*K+1]
        sampled_feature = torch.stack(sampled_feature,dim=4)
        sampled_feature = sampled_feature.flatten(start_dim=3,end_dim=4)
