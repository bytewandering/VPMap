...
def get_corr_rot_angle(p_3d,
    ...
    """
    p_3d-[3,N],三维点云数据
    p_target-[2,N],校正后的二维点云数据
    P-[3,4]投影矩阵
    rot-
    """

    rot_corr_best = 0
    p_source_norm_rot_best = None
    angle = 0
    p_3d_homo = np.row_stack((p_3d,np.ones((1,p_3d.shape[1]))))
    ...
    T1 = np.array([[1,0,0,-p_3d[0,0]],
                   [0,1,0,-p_3d[1,0]],
                   [0,0,1,-p_3d[2,0]],
                   [0,0,0,1        ]])
    T2 = np.array([[1,0,0,p_3d[0,0]],
                   [0,1,0,p_3d[1,0]],
                   [0,0,1,p_3d[2,0]],
                   [0,0,0,1        ]])
    ...
    fig1_ax1.plot(p_target_norm[0],p_target_norm[1])
    fig1_ax2.plot(p_target_norm[0],p_target_norm[1])


    """画三维图"""

    fig2 = plt.figure()
    fig2_ax1 = fig2.add_subplot(111,projection='3d')

    ax_min = np.floor(p_3d[0:3].min(axis = 1)/10)*10
    ax_max = np.ceil(p_3d[0:3].max(axis = 1)/10)*10

    fig2_ax1.set_xticks(np.linspace(ax_min[0], ax_max[0], int((ax_max[0]-ax_min[0])/5))+1)
    fig2_ax1.set_yticks(np.linspace(ax_min[1], ax_max[1], int((ax_max[1]-ax_min[1])/5))+1)
    fig2_ax1.set_zticks(np.linspace(ax_min[2], ax_max[2], int((ax_max[2]-ax_min[2])/5))+1)
    fig2_ax1.set_box_aspect([ax_max[0] - ax_min[0],ax_max[1] - ax_min[1],ax_max[2] - ax_min[2]])

    fig2_ax1.plot([p_3d[0,0],p_3d[0,0]+100*rot_axis[0]],
                  [p_3d[1,0],p_3d[1,0]+100*rot_axis[1]],
                  [p_3d[2,0],p_3d[2,0]+100*rot_axis[2]])

    

    for theta in np.arange(0,M+1)*2*np.pi/M:

        #旋转三维点云
        """这里的旋转矩阵有问题，旋转轴不太对，需要再经过一个平移矩阵"""
        # R[0:3,0:3] = I + np.sin(theta)*K + (1-np.cos(theta))*K**2
        ...
        R = np.array([
            [cos_theta + ux**2 * C, ux * uy * C - uz * sin_theta, ux * uz * C + uy * sin_theta, 0],
            [uy * ux * C + uz * sin_theta, cos_theta + uy**2 * C, uy * uz * C - ux * sin_theta, 0],
            [uz * ux * C - uy * sin_theta, uz * uy * C + ux * sin_theta, cos_theta + uz**2 * C, 0],
            [0,0,0,1]
        ])

        p1 = T2@R@T1@p_3d_homo

        """造影"""
        p1_proj = P@p1
        p_source = p1_proj[0:2]/np.expand_dims(p1_proj[-1],axis=0)
        ...
        """矫正坐标系"""
        N = np.min([p_source_norm.shape[1],p_target_norm.shape[1]])
        corr1 = p_source_norm[0,0:N]@p_target_norm[0,0:N] + p_source_norm[1,0:N]@p_target_norm[1,0:N]
        corr2 = -p_source_norm[0,0:N]@p_target_norm[0,0:N] + p_source_norm[1,0:N]@p_target_norm[1,0:N]
        corr3 = p_source_norm[0,0:N]@p_target_norm[0,0:N] - p_source_norm[1,0:N]@p_target_norm[1,0:N]
        corr4 = -p_source_norm[0,0:N]@p_target_norm[0,0:N] - p_source_norm[1,0:N]@p_target_norm[1,0:N]

        corr_list = np.array([corr1,corr2,corr3,corr4])
        if np.argmax(corr_list) == 0:
            pass
        elif np.argmax(corr_list) == 1:
            p_source_norm[0] = -p_source_norm[0]
        elif np.argmax(corr_list) == 2:
            p_source_norm[1] = -p_source_norm[1]
        elif np.argmax(corr_list) == 3:
            p_source_norm = -p_source_norm
        ...
        #计算相关性,这里需要修改一下
        N = np.min([p_source_norm_rot.shape[1],p_target_norm.shape[1]])
        rot_corr = p_source_norm_rot[1,0:N]@p_target_norm[1,0:N].T + p_source_norm_rot[0,0:N]@p_target_norm[0,0:N].T

        if rot_corr>rot_corr_best:
            rot_corr_best = rot_corr
            p_source_norm_rot_best = p_source_norm_rot
            angle = theta

        plot1, = fig1_ax1.plot(p_source_norm[0],p_source_norm[1])
        plot2, = fig1_ax2.plot(p_source_norm_rot[0],p_source_norm_rot[1])
        plot_3d, = fig2_ax1.plot(p1[0],p1[1],p1[2])
    ...
    fig1_ax2.plot(p_source_norm_rot_best[0],p_source_norm_rot_best[1])
    fig2_ax1.plot(p1[0],p1[1],p1[2])
    return p_source_norm_rot_best, angle





def similarity_DTW(p_source, 
                   p_target,
                   P):

    """
    Input: 
    p_source-ndarray[3,N1]
    ...
    #计算source的主方向
    p_source_zero_norm = p_source-np.expand_dims(p_source[:,0],axis=-1)
    rot_axis = np.mean(p_source_zero_norm,axis=1)
    ...
    #获取branch绕主轴的的最佳旋转角度
    p_source_norm,best_angle = get_corr_rot_angle(p_source,
                                                  p_target_norm,
                                                  P,
                                                  rot_axis)
    ...
    fig4_ax2.plot(-p_source_norm[1],-p_source_norm[0])
...
def branch_similarity_DTW(source:list[np.ndarray], target:list[np.ndarray], P:np.ndarray):
    """
    Input:
    source - list[ndarray[3+1,K1],ndarray[3+1,K2],...]
    target - list[ndarray[2+1,K1'],ndarray[2+1,K2'],...]
    Output:
    ...
    for i in range(num_branches):
        similarity_matrix = similarity_DTW(source[i][0:-1], target[i][0:-1],P)
...
def ICB(array_3d,
    ...
    while(1):
        ...
        #这里的相似度计算要改成三维点云和二维点云中对应的branch之间计算相似度，计算过程虽然需要投影，但是可以将三维branch旋转，
        # 使得投影后的branch和二维branch在低频分量的幅值接近
        branches_cor_similarity_matrixes = branch_similarity_DTW(branches_3d_points_volume,
                                                                 branches_2d_cor_points_pixel,
                                                                 P_cor)
        ...
        branches_sag_similarity_matrixes = branch_similarity_DTW(branches_3d_points_volume,
