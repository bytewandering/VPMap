...
class DSC(object):
    ...
    def _coordinate_map_3D(self, offset, if_offset):
        ...
        x_offset, y_offset = torch.split(offset, self.num_points, dim=1)
        ...
        """初始化采样点位置"""
        center_x = torch.arange(0,W)
        center_y = torch.arange(0,H)
        #[W,H]
        center_x,center_y = torch.meshgrid([center_y,center_x])
        center_x = center_x.unsqueeze(0).unsqueeze(0).repeat([B,2*K+1,1,1]).to(device)
        center_y = center_y.unsqueeze(0).unsqueeze(0).repeat([B,2*K+1,1,1]).to(device)
        
        """根据offset图计算每个采样点的偏移量"""
        if self.morph == 0:
            offset_y = torch.range(-K,K).unsqueeze(0).unsqueeze(-1).unsqueeze(-1).repeat([B,1,W,H]).to(device)
            offset_x = torch.zeros_like(offset_y).to(device)
            for i in range(0,K):
                offset_x[:,K+i+1,:,:] = offset_x[:,K+i,:,:]+x_offset[:,K+i+1,:,:]
                offset_x[:,K-i-1,:,:] = offset_x[:,K+i,:,:]+x_offset[:,K-i-1,:,:]
        elif self.morph == 1:
            offset_x = torch.range(-K,K).unsqueeze(0).unsqueeze(-1).unsqueeze(-1).repeat([B,1,W,H]).to(device)
            offset_y = torch.zeros_like(offset_x).to(device)
            for i in range(0,K):
                offset_y[:,K+i+1,:,:] = offset_y[:,K+i,:,:]+y_offset[:,K+i+1,:,:]
                offset_y[:,K-i-1,:,:] = offset_y[:,K-i,:,:]+y_offset[:,K-i-1,:,:]
        
        
        """根据offset确定采样点的位置"""
        #[B,2*K+1,W,H]
        sampled_coor_x = center_x+offset_x
        sampled_coor_y = center_y+offset_y
