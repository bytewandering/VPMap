...
if __name__ == "__main__":
    ...
    pred_dir_path = "F:/3D_2D/VAPPF_info/label+ICP"
    #输入的分割图的名称
    # test_dir_path = "F:/3D_2D/VAPPF/pred_data/UNet/predictions/test"
    test_dir_path = "F:/3D_2D/VAPPF/test_data"
    image_dir_path = "F:/3D_2D/VAPPF/test_data"
    ...
    # for i in range(3,4):
        ...
        test_dir = test_dir_path + "\\" +dir_list[i]

        print("########registration ",dir_list[i],"########")
        
        """读取三维影像"""
        #读取影像数据
        dsa3d_seg_path = image_dir+"/3D/3D_DSA_seg.nii.gz"
        ...
        """读取cor影像"""
        #读取影像数据
        dsa2d_seg_cor_path = test_dir+"/cor/label.png"
        ...
        """读取sag影像"""
        #读取影像数据
        dsa2d_seg_sag_path = test_dir+"/sag/label.png"
        ...
        """读取预测的json文件"""
        with open(os.path.join(pred_dir,"result.json"),"r") as pred_file:
            pred_json = json.load(pred_file)
        with open(os.path.join(image_dir,"parameters.json"),"r") as image_file:
            image_json = json.load(image_file)

        # w0 = np.array(pred_json["w_init"])
        w0 = [-np.pi/2,1.4,0,-10,10,0]
        # w0 = [-1.66,0,0,-0.5,-1.5,-1]
        T0 = w_to_T(w0)
        w_ground_truth = np.array(image_json["w_ground_truth"])
        ...
        plt.tight_layout()
        # plt.axis("off")
        img_cor_path = image_dir+"/cor/img.png"
        img_cor = cv2.imread(img_cor_path)
        plt.imshow(img_cor)

        cmap1 = ListedColormap(["none","blue"])
        ...
        plt.imshow(seg_ia_array_dsa2d_cor_proj_after_reg.transpose([1,0,2]),
                    ...
                    vmin=np.min(seg_ia_array_dsa2d_cor_proj_after_reg), 
                    vmax=np.max(seg_ia_array_dsa2d_cor_proj_after_reg))
        plt.savefig(pred_dir+"/"+"/proj_cor_groundtruth.png", dpi=512, bbox_inches='tight')
        plt.close()
        ...
        plt.tight_layout()
        # plt.axis("off")
        img_sag_path = image_dir+"/sag/img.png"
        img_sag = cv2.imread(img_sag_path)
        plt.imshow(img_sag)
        ...
        plt.imshow(seg_ia_array_dsa2d_sag_proj_after_reg.transpose([1,0,2]),
                    ...
                    vmin=np.min(seg_ia_array_dsa2d_sag_proj_after_reg), 
                    vmax=np.max(seg_ia_array_dsa2d_sag_proj_after_reg))
        plt.savefig(pred_dir+"/"+"/proj_sag_groundtruth.png", dpi=512, bbox_inches='tight')
        plt.close()
