...
from ..utilities.network_initialization import InitWeights_He
...
class ResidualMambaEncoder(nn.Module):
    def __init__(self,
        ...
        for s in range(n_stages):
            feature_map_sizes.append([i // j for i, j in zip(feature_map_size, [strides[s]]*2)])
...
class UNetResDecoder(nn.Module):
    def __init__(self,
        ...
        if isinstance(n_conv_per_stage, int):
            n_conv_per_stage = [n_conv_per_stage] * n_stages_encoder
        assert len(n_conv_per_stage) == n_stages_encoder, "n_conv_per_stage must have as many entries as we have " \
...
class UMambaEnc(nn.Module):
    def __init__(self,
        ...
        for s in range(math.ceil((n_stages) / 2 + 0.5), n_stages):
        ...
        assert len(n_conv_per_stage_decoder) == n_stages, "n_conv_per_stage_decoder must have one less entries " \
